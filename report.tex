...
droplet. Plus, $S$ is saturation ratio. So if $S$ is less than 1,
$\frac{dr}{dt}$ is less than 0, which describes the evaporation of a cloud
drop. In the other way when $S$ is greater than 1, it describes the
condensation of a cloud drop.
...
vapor diffused towards the drop and heat diffused away from the drop.

In sum, the droplet growth rate is a function of saturation ratio over the
balance force between diffusion and convection.
